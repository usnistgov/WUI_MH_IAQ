...
def calculate_time_since_garage_closed(df, datetime_column, garage_closed_time):
    ...
    if DEBUG:
        ...
        print(
            f"  Time Since Garage Closed range: {time_range['min']:.2f} to {time_range['max']:.2f} hours"
        )
...
def calculate_rolling_average_burn3(data, datetime_column):
    ...
    for column in numeric_columns:
        rolling_avg_data[column] = (
            ...
            .mean()
            .astype(data_indexed[column].dtype)
        )
        ...
        try:
            ...
            for burn_number in quantaq_burns:
                ...
                filtered_data = quantaq_b_data[
                    quantaq_b_data["Date"] == burn_date.date()
                ].copy()
        ...
        try:
            ...
            for burn_number in quantaq_burns:
                ...
                filtered_data = quantaq_k_data[
                    quantaq_k_data["Date"] == burn_date.date()
                ].copy()
...
def create_plots_for_burn(burn_number):
    ...
    if bedroom_data:
        ...
        for instrument_name, instrument_data in bedroom_data.items():
            ...
            if instrument_name == "SMPS" and DEBUG:
                ...
                if time_col in instrument_data.columns:
                    ...
                    print(
                        f"{time_col} stats: min={time_stats['min']:.2f}, mean={time_stats['mean']:.2f}, max={time_stats['max']:.2f}"
                    )
                    # Check for NaN values
                    nan_count = instrument_data[time_col].isna().sum()
                    print(
                        f"NaN count in {time_col}: {nan_count}/{len(instrument_data)} ({nan_count/len(instrument_data)*100:.1f}%)"
                    )
        ...
        output_file_path = os.path.join(
            output_dir, f"{burn_number}_bedroom_particle_count_comparison.html"
        )
    ...
    if kitchen_data:
        ...
        for instrument_name, instrument_data in kitchen_data.items():
            ...
            def extract_size(col_name):
                try:
                    # Extract the size range (e.g., "Ʃ0.5-1.0µm" -> [0.5, 1.0])
                    parts = (
                        col_name.replace("Ʃ", "").replace("µm (#/cm³)", "").split("-")
                    )
                    return float(parts[0])
                except (ValueError, IndexError):
                    return float("inf")  # Put columns with parsing errors at the end
        ...
        output_file_path = os.path.join(
            output_dir, f"{burn_number}_kitchen_particle_count_comparison.html"
        )
        output_file(output_file_path)
