"""
WUI Temperature and Relative Humidity Comparison Analysis

This script analyzes temperature and relative humidity (RH) measurements from
multiple sensors throughout the test house during wildland-urban interface
smoke experiments. It evaluates environmental conditions and their spatial
and temporal variations.

Key Features:
    - Multi-sensor temperature comparison
    - Multi-sensor RH comparison
    - Spatial variation analysis (bedroom, kitchen, morning room)
    - Temporal evolution during burn events
    - Cross-correlation between T and RH

Sensor Locations:
    - Bedroom2 (primary)
    - Kitchen
    - Morning room
    - Living room (when available)

Analysis Components:
    1. Temperature time series overlay
    2. RH time series overlay
    3. T-RH phase space plots
    4. Spatial uniformity assessment
    5. Event-triggered changes (door closure, CR Box activation)

Research Applications:
    - Verify environmental stability during experiments
    - Assess heating/cooling from smoke infiltration
    - Evaluate RH changes from combustion products
    - Support humidity correction of PM data

Metrics Calculated:
    - Mean temperature and RH by location
    - Standard deviation (temporal variability)
    - Coefficient of variation (spatial uniformity)
    - Rate of change during events

Outputs:
    - Synchronized T and RH time series plots
...
Configuration:
    - Directory path (Windows-specific)
...
for burn in burn_dates.keys():
    ...
    p.add_layout(
        LinearAxis(y_range_name="RH", axis_label="Relative Humidity [%]"), "right"
    )
    ...
    for rh_col in colors:
        if rh_col in data.columns:
            p.line(
                ...
                data[rh_col],
                color=colors[rh_col],
                legend_label=legend_labels.get(rh_col, rh_col),
                line_width=1,
                y_range_name="RH",
            )
