...
from bokeh.models import ColumnDataSource, Band, Label, Arrow, OpenHead, CrosshairTool, Span, Legend, LegendItem
...
for label, data in decay_data.items():
    ...
    p.add_layout(Label(x=label_x_max, y=label_y_max, text=f"Max: {max_y:.1f} (µg/m³)", text_font='Calibri', text_font_size='12pt', text_align='left', text_baseline='bottom'))
    
    # Add an arrow pointing to the maximum value
    arrow_max = Arrow(end=OpenHead(size=10, line_color='black'), line_color='black', x_start=label_x_max, y_start=label_y_max, x_end=max_x, y_end=max_y)
