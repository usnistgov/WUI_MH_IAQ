...
from bokeh.models import ColumnDataSource, DatetimeTickFormatter, HoverTool, TapTool, LinearAxis, Range1d, LogTicker
...
cr_box_activation_time_str = burn_log.loc[burn_log['Burn ID'] == burn_number, 'CR Box on'].values[0]
cr_box_activation_time_full = pd.to_datetime(f"{burn_date} {cr_box_activation_time_str}") if pd.notna(cr_box_activation_time_str) else None

# Extract and convert the time from the 'garage closed' column
door_closed_time_str = burn_log.loc[burn_log['Burn ID'] == burn_number, 'garage closed'].values[0]
door_closed_time_full = pd.to_datetime(f"{burn_date} {door_closed_time_str}") if pd.notna(door_closed_time_str) else None

#%% 
# Load VOCUS data for burn10
vocus_path = os.path.join(absolute_path, 'burn_data/VOCUS/May31_2024_VOCUSdata.xlsx')
vocus_data = pd.read_excel(vocus_path, parse_dates=['datetime_EDT'], index_col='datetime_EDT')
...
aerotrak_path = os.path.join(absolute_path, 'burn_data/aerotraks/kitchen/all_data.xlsx') #kitchen data
aerotrak_data = pd.read_excel(aerotrak_path, parse_dates=['Date and Time'], index_col='Date and Time')

# Filter for the given burn date
aerotrak_data = aerotrak_data[aerotrak_data.index.date == pd.to_datetime(burn_date).date()]
...
if volume_column in aerotrak_data.columns:
    ...
    for col in aerotrak_data.columns:
        if 'Cumul (#)' in col:
            ...
            aerotrak_data.rename(columns={col + ' (cm³)': col.replace('Cumul (#)', 'Cumul (#/cm³)'), 'Cumul (#/cm³)': col}, inplace=True)
        elif 'Diff (#)' in col:
            aerotrak_data[col + ' (cm³)'] = aerotrak_data[col] / volume_liters
            aerotrak_data.rename(columns={col + ' (cm³)': col.replace('Diff (#)', 'Diff (#/cm³)'), 'Diff (#/cm³)': col}, inplace=True)
...
quantaq_path = os.path.join(absolute_path, 'burn_data/quantaq/MOD-PM-00197-a6dd467a147a4d95a7b98a8a10ab4ea3.csv') #kicten data
quantaq_data = pd.read_csv(quantaq_path, parse_dates=['timestamp_local'], index_col='timestamp_local')
...
bins = {
    ...
    'Ʃ20-40 µm (#/cm³)': ['bin17', 'bin18', 'bin19', 'bin20', 'bin21', 'bin22', 'bin23']
}
...
purpleair_data = pd.read_excel(purpleair_path, sheet_name='(P2)kitchen', parse_dates=['DateTime'], index_col='DateTime')
...
purpleair_data = purpleair_data[purpleair_data.index.date == pd.to_datetime(burn_date).date()]
...
# Plot Aerotrak data
source_aerotrak = ColumnDataSource(data={
    'datetime': aerotrak_data.index,
    'value': aerotrak_data[aerotrak_cumul_col].values
})
p.line(x='datetime', y='value', source=source_aerotrak, line_width=2, color=aerotrak_color[0], legend_label='AeroTrak total PM (#/cm³)')

# Plot PurpleAir data
source_purpleair = ColumnDataSource(data={
    'datetime': purpleair_data.index,
    'value': purpleair_data['PM2.5 (µg/m³)'].values
})
p.line(x='datetime', y='value', source=source_purpleair, line_width=2, color=purpleair_color[0], legend_label='PurpleAir PM2.5 (µg/cm³)')

# Plot QuantAQ data without filtering
if quantaq_sum_col in quantaq_data.columns:
    source_quantaq = ColumnDataSource(data={
        'datetime': quantaq_data.index,
        'value': quantaq_data[quantaq_sum_col].values
    })
    p.line(x='datetime', y='value', source=source_quantaq,
            legend_label=f'QuantAQ {quantaq_sum_col}', line_width=2,
            color=quantaq_color[0], line_dash='dashed')
...
# Plot VOCUS data
source_vocus = ColumnDataSource(data={
    'datetime': vocus_data.index,
    'value': vocus_data['C3H4N_cps'].values
})
...
p.line(x='datetime', y='value', source=source_vocus, line_width=2, color=smps_color[0], legend_label='VOCUS C3H4N (cps)', line_dash='solid', y_range_name='vocus_range')
