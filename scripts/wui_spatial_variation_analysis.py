...
INSTRUMENT_CONFIG = {
    'AeroTrakB': {
        ...
        'time_shift': 2.16,
        'process_pollutants': ['PM0.5 (µg/m³)', 'PM1 (µg/m³)', 'PM3 (µg/m³)', 
...
def calculate_rolling_average_burn3(data):
    ...
    data.loc[data['Date'] == burn3_date, pollutant_columns] = burn3_data[pollutant_columns]
...
def process_aerotrak_data(file_path, instrument='AeroTrakB'):
    ...
    for i, channel in enumerate(size_channels):
        if channel in size_values:
            ...
            if cumul_col in aerotrak_data.columns:
                ...
                aerotrak_data[new_cumul_col] = cumul_data / volume_cm  # Convert to #/cm³
                
                # Create new PM concentration column from the Diff column if it was created
                pm_column_name = f'PM{next_size_value} (µg/m³)'
                if diff_col in aerotrak_data.columns and new_diff_col_µg_m3 in aerotrak_data.columns:
    ...
    for i in range(len(cumulative_columns)):
        if i == 0 and len(pm_columns) > 0:
            aerotrak_data[cumulative_columns[i]] = pd.to_numeric(aerotrak_data[pm_columns[i]], errors='coerce')
        elif i < len(pm_columns):
            # Sum current PM column with the previous cumulative column
            aerotrak_data[cumulative_columns[i]] = pd.to_numeric(aerotrak_data[pm_columns[i]], errors='coerce').add(
                pd.to_numeric(aerotrak_data[cumulative_columns[i - 1]], errors='coerce'), fill_value=0
            )
    ...
    for col in cumulative_columns:
        if col in filtered_aerotrak_data.columns:
            filtered_aerotrak_data[col] = pd.to_numeric(filtered_aerotrak_data[col], errors='coerce')
    ...
    if baseline_values:
        ...
        for pollutant, (baseline_val, _) in baseline_values.items():
            ...
            if pollutant in filtered_aerotrak_data.columns:
                ...
                filtered_aerotrak_data[pollutant] = pd.to_numeric(filtered_aerotrak_data[pollutant], errors='coerce')
                # Subtract baseline from measurements
                filtered_aerotrak_data[pollutant] = filtered_aerotrak_data[pollutant] - baseline_val
                # Set negative values to 0
                filtered_aerotrak_data[pollutant] = filtered_aerotrak_data[pollutant].clip(lower=0)
    
    # Calculate Time Since Garage Closed for each burn
    for burn_id in burn_ids:
        burn_mask = filtered_aerotrak_data['Date'] == pd.to_datetime(
            burn_log[burn_log['Burn ID'] == burn_id]['Date'].values[0] if burn_id in burn_log['Burn ID'].values else pd.NaT
        ).date()
        ...
        if burn_mask.any() and burn_id in burn_log['Burn ID'].values:
            ...
            if pd.notna(garage_closed_time_str) and garage_closed_time_str != 'n/a':
                ...
                filtered_aerotrak_data.loc[burn_mask, 'Time Since Garage Closed (hours)'] = (
                    filtered_aerotrak_data.loc[burn_mask, 'Date and Time'] - garage_closed_time
...
def process_quantaq_data(file_path, instrument='QuantAQB'):
    ...
    rename_dict = {'pm1': 'PM1 (µg/m³)', 'pm25': 'PM2.5 (µg/m³)', 'pm10': 'PM10 (µg/m³)'}
            ...
            if burn_mask.any():
                ...
                if pd.notna(garage_closed_time_str) and garage_closed_time_str != 'n/a':
                    ...
                    filtered_data.loc[burn_mask, 'Time Since Garage Closed (hours)'] = (
                        filtered_data.loc[burn_mask, 'timestamp_local'] - garage_closed_time
                    ).dt.total_seconds() / 3600
...
def calculate_average_ratio_and_rsd(bedroom_data, morning_data, burn_id, pm_size, 
                                   datetime_col_b, datetime_col_m, analysis_duration_hours=2):
    """
    ...
    bedroom_resample = bedroom_resample.set_index(datetime_col_b)[pm_size].resample('1T').mean()
    morning_resample = morning_resample.set_index(datetime_col_m)[pm_size].resample('1T').mean()
    ...
    merged = merged[(merged[f'{pm_size}_morning'] > 0) & (merged[f'{pm_size}_bedroom'] > 0)]
...
def analyze_spatial_variation():
    ...
    try:
        ...
        print(f"AeroTrakB PM columns: {[col for col in aerotrakb_data.columns if 'PM' in col and '(µg/m³)' in col]}")
        print(f"AeroTrakK PM columns: {[col for col in aerotrakk_data.columns if 'PM' in col and '(µg/m³)' in col]}")
    ...
    aerotrak_pm_sizes = ['PM0.5 (µg/m³)', 'PM1 (µg/m³)', 'PM3 (µg/m³)', 
                         'PM5 (µg/m³)', 'PM10 (µg/m³)', 'PM25 (µg/m³)']
    ...
    for burn_id in burn_log['Burn ID']:
            ...
            for pm_size in aerotrak_pm_sizes:
                try:
                    ...
                    avg_ratio, rsd = calculate_average_ratio_and_rsd(
                        aerotrakb_data, aerotrakk_data, burn_id, pm_size,
                    ...
                    if peak_ratio is not None or avg_ratio is not None:
                        ...
                        peak_str = f"{peak_ratio:.3f}" if peak_ratio is not None else "N/A"
                        avg_str = f"{avg_ratio:.3f}" if avg_ratio is not None else "N/A"
                        rsd_str = f"{rsd:.1f}" if rsd is not None else "N/A"
                        
                        print(f"    {pm_size}: R_I={peak_str}, R_ave={avg_str}, RSD={rsd_str}%")
                    else:
        ...
        if burn_num >= 4 and quantaqb_data is not None and quantaqk_data is not None:
            ...
            for pm_size in quantaq_pm_sizes:
                try:
                    # Calculate peak ratio
                    peak_ratio = calculate_peak_ratio(peak_data, burn_id, 'QuantAQ', pm_size)
                    
                    # Calculate average ratio and RSD
                    avg_ratio, rsd = calculate_average_ratio_and_rsd(
                        quantaqb_data, quantaqk_data, burn_id, pm_size,
    ...
    if aerotrak_df.empty and quantaq_df.empty:
        print("WARNING: No results to save - both AeroTrak and QuantAQ DataFrames are empty")
    ...
    if not aerotrak_df.empty:
        ...
        # Fixed f-string formatting
        print(f"  Mean Peak Ratio: {peak_mean:.3f}" if not pd.isna(peak_mean) else "  Mean Peak Ratio: N/A")
        print(f"  Mean Average Ratio: {avg_mean:.3f}" if not pd.isna(avg_mean) else "  Mean Average Ratio: N/A")
        print(f"  Mean RSD: {rsd_mean:.1f}%" if not pd.isna(rsd_mean) else "  Mean RSD: N/A")
