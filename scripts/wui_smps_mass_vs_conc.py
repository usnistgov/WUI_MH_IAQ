...
SMPS_df['Datetime'] = pd.to_datetime(SMPS_df['Date'].astype(str) + ' ' + SMPS_df['Start Time'].astype(str))
...
SMPS_path2 = './burn_data/smps/MH_apollo_bed_05010024_MassConc.xlsx'  # Corrected file path
SMPS_df2 = pd.read_excel(SMPS_path2, sheet_name='all_data')

# Combine 'Date' and 'Start Time' into a single datetime column
SMPS_df2['Datetime'] = pd.to_datetime(SMPS_df2['Date'].astype(str) + ' ' + SMPS_df2['Start Time'].astype(str))
...
def sum_columns2(df, ranges):
    summed_data2 = {}
    for start, end in ranges:
        col_range2 = [col for col in df.columns if str(col).replace('.', '', 1).isdigit() and start <= float(col) <= end]
...
color_scale_orange = ['#ff7f0e', '#fbad4c', '#fad289', '#fff3cc']  # Shades of orange for SMPS_df
color_scale_blue = ['#003e58', '#417a8e', '#7ebbc6', '#c3ffff']  # Shades of blue for SMPS_df2
...
for (label, data), color in zip(summed_data.items(), color_scale_orange):
    p.line(SMPS_df['min_from_peak'], data, legend_label=label, line_width=2, color=color)

# Add a secondary y-axis for mass concentration data (log scale)
p.extra_y_ranges = {"mass_conc": Range1d(start=0.001, end=10**5)}  # Initial range to ensure visibility
p.add_layout(LogAxis(y_range_name="mass_conc", axis_label='Particulate Matter (mass concentration)'), 'right')

# Plot SMPS_df2 data on the right y-axis
for (label, data), color in zip(summed_data2.items(), color_scale_blue):
    p.line(SMPS_df2['min_from_peak'], data, legend_label=label, line_width=2, color=color, y_range_name="mass_conc")
