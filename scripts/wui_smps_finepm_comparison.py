...
range_boundaries = [(9, 25), (26, 50), (51,75), (76, 100), (101, 125), (126, 150), (151, 175), (176,200), (201, 250)]
...
for start, end in range_boundaries:
    numeric_cols = combined_smps_data.columns[pd.to_numeric(combined_smps_data.columns, errors='coerce').notnull()]
    col_names = numeric_cols[(numeric_cols.astype(float) >= start) & (numeric_cols.astype(float) <= end)].tolist()
    if col_names:
        combined_smps_data[f'Ʃ{start}-{end}nm (#/cm³)'] = combined_smps_data[col_names].sum(axis=1)
...
for i, col in enumerate(valid_smps_cols[valid_smps_cols.str.contains('Ʃ')]):
    ...
    if not filtered_smps.empty:
        source_smps = ColumnDataSource(data={
            'datetime': filtered_smps.index,
            'value': filtered_smps.values
        })
        p.line(x='datetime', y='value', source=source_smps,
                legend_label=col.replace(' (#/cm³)', ''), line_width=2,
                color=smps_color[i % len(smps_color)], line_dash='solid')
...
hourly_ticks = pd.date_range(start=combined_smps_data.index.min(), end=combined_smps_data.index.max(), freq='60min')
...
for i, col in enumerate(valid_smps_cols[valid_smps_cols.str.contains('Ʃ')]):
    ...
    if not filtered_smps.empty:
        ...
        p.line(x='datetime', y='value', source=source_smps,
                legend_label=col.replace(' (#/cm³)', ''), line_width=1.5,
                color=smps_color[i % len(smps_color)], line_dash='solid')
...
p.x_range.start = pd.Timestamp('2024-05-27 08:00:00')  # Start from 05/27/2024 8:00:00 AM
