...
def process_aerotrak_data(file_path, instrument):
    ...
    filtered_aerotrak_data = aerotrak_data[aerotrak_data['Date'] == burn4_date.date()].copy()

    # Apply time shift for burn4
    filtered_aerotrak_data = apply_time_shift(filtered_aerotrak_data, instrument, 'burn4', burn4_date)
    ...
    garage_closed_time_str = burn_log[burn_log['Burn ID'] == 'burn4']['garage closed'].iloc[0]
    garage_closed_time = create_naive_datetime(burn4_date.date(), garage_closed_time_str)
...
def calculate_decay_parameters_burn4(data, instrument, burn_id='burn4'):
    ...
    garage_closed_time_str = burn_log[burn_log['Burn ID'] == burn_id]['garage closed'].iloc[0]
    ...
    for pollutant in pollutants:
        ...
        valid_data['filtered_concentration'] = pd.to_numeric(valid_data[pollutant], errors='coerce').where(
            pd.to_numeric(valid_data[pollutant], errors='coerce') > 0, other=np.nan)
        
        try:
            numeric_values = pd.to_numeric(valid_data['filtered_concentration'], errors='coerce')
        ...
        valid_data['rolling_derivative'] = valid_data['log_concentration'].diff().rolling(window=5).mean()
        
        # Initialize decay start time
        decay_start_time = max(0, max_time)
        
        # Get CR Box on time
        cr_box_on_time_str = burn_log[burn_log['Burn ID'] == burn_id]['CR Box on'].iloc[0]
        ...
        for idx in range(max_index_valid + 1, len(valid_data)):
            if idx >= max_index_valid + 5:  # Ensure enough points for rolling mean
                rolling_mean = valid_data['rolling_derivative'].iloc[idx-3:idx+1].mean()
                stability_threshold = 0.1
                if abs(valid_data['rolling_derivative'].iloc[idx] - rolling_mean) < stability_threshold:
                    potential_start_time = valid_data['Time Since Garage Closed (hours)'].iloc[idx]
...
def create_burn4_comparison_plot():
    ...
    aerotrakb_data = process_aerotrak_data(INSTRUMENT_CONFIG['AeroTrakB']['file_path'], 'AeroTrakB')
    aerotrakk_data = process_aerotrak_data(INSTRUMENT_CONFIG['AeroTrakK']['file_path'], 'AeroTrakK')
    ...
    for pollutant in pollutants:
        if pollutant in aerotrakb_data.columns:
            ...
            p.line('Time Since Garage Closed (hours)', pollutant, source=source_b, 
                   legend_label=legend_label, line_width=1.5, color=color, line_dash='dashed')
    ...
    for pollutant in pollutants:
        if pollutant in aerotrakk_data.columns:
            ...
            color = POLLUTANT_COLORS.get(pollutant, 'gray')
            p.line('Time Since Garage Closed (hours)', pollutant, source=source_k, 
                   legend_label=legend_label, line_width=1.5, color=color, line_dash='solid')
    
    # Add vertical line for garage closed
    p.line(x=[0] * 2, y=[10**-2, 10**4], line_color='black',
    ...
    if pd.notna(cr_box_on_time):
        cr_box_on_time_since_garage_closed = (cr_box_on_time - garage_closed_time).total_seconds() / 3600
        p.line(x=[cr_box_on_time_since_garage_closed] * 2, y=[10**-2, 10**4], 
               line_color='black', line_width=1, line_dash='dashed', legend_label='CR Box on')
            ...
            for pollutant in pollutants:
                if pollutant in decay_parameters['burn4'][instrument]:
                    ...
                    try:
                        ...
                        print(f"Error plotting decay curve for {pollutant} in {instrument}: {str(e)}")
                        continue
