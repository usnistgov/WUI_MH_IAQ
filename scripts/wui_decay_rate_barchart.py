...
# Define needed paths
absolute_path = 'C:/Users/nml/OneDrive - NIST/Documents/NIST/WUI_smoke/burn_data/burn_calcs'
...
def create_baseline_corrected_data(data_by_instrument, baseline_calculator, pollutant_type, burns_to_process, exclude_instruments=None):
    ...
    for burn in burns_to_process:
        ...
        for instrument in corrected_data:
            if burn in corrected_data[instrument]:
                ...
                baseline_corrected_uncertainties.append(corrected_data[instrument][burn]['uncertainty'])
        ...
        if baseline_corrected_values:
            ...
            sem = np.std(baseline_corrected_values) / np.sqrt(len(baseline_corrected_values))
            instrument_uncertainty_contribution = np.sqrt(np.sum(np.array(baseline_corrected_uncertainties)**2)) / len(baseline_corrected_values)
...
def perform_filter_count_anova(data_by_instrument, baseline_calculator, pollutant_type, output_file):
    ...
    if len(groups) >= 2:
        ...
        for i, burn in enumerate(filter_count_burns):
            if burn in mean_data:
                output_file.write(f"  {filter_count_labels[i]}: {mean_data[burn]['decay']:.4f} ± {mean_data[burn]['uncertainty']:.4f} h⁻¹\n")
        ...
        return {
            ...
            'significant': is_significant,
            'sig_indicator': sig_indicator,
            'groups': {filter_count_labels[i]: mean_data[burn] for i, burn in enumerate(filter_count_burns) if burn in mean_data}
        }
    
    return {}

def perform_new_vs_used_anova(data_by_instrument, baseline_calculator, pollutant_type, output_file):
    ...
    if new_values and used_values:
        ...
        output_file.write(f"  New filters: n={len(new_values)}, mean={np.mean(new_values):.4f} h⁻¹\n")
        output_file.write(f"  Used filters: n={len(used_values)}, mean={np.mean(used_values):.4f} h⁻¹\n")
        output_file.write(f"\n  F-statistic: {f_stat:.4f}\n")
        output_file.write(f"  P-value: {p_value:.6f}\n")
...
def perform_merv_comparison_anova(data_by_instrument, baseline_calculator, pollutant_type, output_file):
    ...
    if merv12a_values and merv13_values:
        ...
        output_file.write(f"  MERV12A: n={len(merv12a_values)}, mean={np.mean(merv12a_values):.4f} h⁻¹\n")
        output_file.write(f"  MERV13: n={len(merv13_values)}, mean={np.mean(merv13_values):.4f} h⁻¹\n")
...
def perform_two_way_anova_filter_analysis(data_by_instrument, baseline_calculator, pollutant_type, output_file):
...
def compare_anova_vs_instrument_uncertainties(data_by_instrument, baseline_calculator, pollutant_type, output_file):
        ...
        if 'PM0.4' in summary_results:
            ...
            if 'filter_count' in pm04_results:
                ...
                for result in pm04_results['filter_count']:
                    ...
                    f.write(f"     Mean: {result['mu_a']:.4f}±{result['sd_a']:.4f} vs {result['mu_b']:.4f}±{result['sd_b']:.4f} h⁻¹\n")
                    f.write(f"     Z={result['z_stat']:.4f}, p={result['p_value']:.6f} {result['sig_indicator']}\n")
                ...
                if 'filter_count' in pm_results:
                    ...
                    if 'groups' in result:
                        for label, data in result['groups'].items():
                            f.write(f"   {label}: {data['decay']:.4f}±{data['uncertainty']:.4f} h⁻¹\n")
                    f.write(f"   F={result.get('f_stat', 0):.4f}, p={result.get('p_value', 1):.6f} {result.get('sig_indicator', 'ns')}\n")
...
def create_transposed_bar_chart(pm04_data, pm1_data, pm25_data, pm10_data, config, script_metadata):
    ...
    for pm_type in selected_pm_types:
        ...
        for burn in config['burns']:
            if burn in pm_data:
                ...
                detailed_info.append(f"{label} ({pm_type}): {decay:.3f} ± {uncertainty:.3f} h⁻¹ ({instruments_str})")
    ...
    chart_colors = [colors['AeroTrakB'], colors['PurpleAirK'], colors['SMPS']]  # #003f5c, #665191, #d45087
...
with open(stats_file_path, 'w', encoding='utf-8') as stats_file:
    ...
    pm04_filter_count = perform_pm04_filter_count_ztest(pm04_data_raw, baseline_calculator, stats_file)
    pm04_new_vs_used = perform_pm04_new_vs_used_ztest(pm04_data_raw, baseline_calculator, stats_file)
    pm04_merv = perform_pm04_merv_comparison_ztest(pm04_data_raw, baseline_calculator, stats_file)
    
    summary_results['PM0.4'] = {
        **pm04_filter_count,
        **pm04_new_vs_used,
        **pm04_merv
    }
    ...
    pm1_filter_count = perform_filter_count_anova(pm1_data_raw, baseline_calculator, "PM1", stats_file)
    pm1_new_vs_used = perform_new_vs_used_anova(pm1_data_raw, baseline_calculator, "PM1", stats_file)
    pm1_merv = perform_merv_comparison_anova(pm1_data_raw, baseline_calculator, "PM1", stats_file)
    perform_two_way_anova_filter_analysis(pm1_data_raw, baseline_calculator, "PM1", stats_file)
    compare_anova_vs_instrument_uncertainties(pm1_data_raw, baseline_calculator, "PM1", stats_file)
    ...
    pm25_filter_count = perform_filter_count_anova(pm25_data_raw, baseline_calculator, "PM2.5", stats_file)
    pm25_new_vs_used = perform_new_vs_used_anova(pm25_data_raw, baseline_calculator, "PM2.5", stats_file)
    pm25_merv = perform_merv_comparison_anova(pm25_data_raw, baseline_calculator, "PM2.5", stats_file)
    perform_two_way_anova_filter_analysis(pm25_data_raw, baseline_calculator, "PM2.5", stats_file)
    compare_anova_vs_instrument_uncertainties(pm25_data_raw, baseline_calculator, "PM2.5", stats_file)
    ...
    pm10_filter_count = perform_filter_count_anova(pm10_data_raw, baseline_calculator, "PM10", stats_file)
    pm10_new_vs_used = perform_new_vs_used_anova(pm10_data_raw, baseline_calculator, "PM10", stats_file)
    pm10_merv = perform_merv_comparison_anova(pm10_data_raw, baseline_calculator, "PM10", stats_file)
    perform_two_way_anova_filter_analysis(pm10_data_raw, baseline_calculator, "PM10", stats_file)
    compare_anova_vs_instrument_uncertainties(pm10_data_raw, baseline_calculator, "PM10", stats_file)
...
chart_configs = [
    {
        ...
        'pm_types': ['PM0.4', 'PM2.5'] #all available PM types 'PM0.4', 'PM1', 'PM2.5', 'PM10'
    },
    {
...
for config in chart_configs:
    ...
    if config.get('use_matching_instruments', False):
        ...
        exclude_instruments = [i for i in instruments if i not in matching_instruments and i not in exclude_instruments]
