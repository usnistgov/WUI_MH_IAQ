#%%
"""
SMPS Heatmap Data Visualization Script for WUI Smoke Study
===========================================================
This script processes SMPS data from multiple burns in the WUI smoke study and creates
heatmap visualizations showing particle size distributions over time.

Features:
- Reads Excel files containing SMPS data
- Handles complex file structure with header information in various locations
- Creates consistent heatmaps with fixed time scales for easy comparison
- Processes both number and mass concentration data
- Saves output figures to Paper_figures directory
- Displays plots in notebook during tuning
"""
...
def prepare_data(file_path, date_str):
    ...
    try:
        ...
        try:
            ...
            df_t['datetime'] = pd.to_datetime(df_t['date_part'] + ' ' + df_t['time_str'], errors='coerce')
            ...
            df_t['hours'] = [(t - day_start).total_seconds() / 3600 for t in df_t['mid_datetime']]
            ...
            print(f"Data array shape: {data_array.shape}, non-NaN values: {non_nan_count} ({non_nan_count/(data_array.size)*100:.1f}% of data)")
...
def create_heatmap(data_for_plot, time_hours, size_bins, description, 
                   data_min=None, data_max=None, data_type='numConc',
                   time_start=0, time_end=24, min_size=None, max_size=None):
    """
    ...
    if data_min is None or np.isnan(data_min):
        data_min = float(np.nanmin(data_for_plot)) if np.any(~np.isnan(data_for_plot)) else 0
    if data_max is None or np.isnan(data_max):
        data_max = float(np.nanmax(data_for_plot)) if np.any(~np.isnan(data_for_plot)) else 1
...
def process_single_burn(burn_date, data_type='numConc', time_start=0, time_end=24):
    ...
    # Prepare data
    data_for_plot, time_hours, size_bins, min_size, max_size = prepare_data(file_path, burn_date)
    ...
    print(f"Non-NaN values: {non_nan_count} ({non_nan_count/(data_for_plot.size)*100:.1f}% of data)")
    ...
    try:
        ...
        heatmap = create_heatmap(
            data_for_plot, time_hours, size_bins, description,
            data_min, data_max, data_type,
            time_start, time_end, min_size, max_size
        )
        ...
        output_file_path = f'./Paper_figures/SMPS_{description}_{data_type}_heatmap.html'
...
def process_all_burns(data_type='numConc', time_start=0, time_end=24):
    ...
    for i, (data_for_plot, time_hours, size_bins, min_size, max_size, description) in enumerate(
        zip(all_data_for_plot, all_time_hours, all_size_bins, all_min_sizes, all_max_sizes, all_descriptions)):
