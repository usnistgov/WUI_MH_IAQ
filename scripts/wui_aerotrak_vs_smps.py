...
columns_to_drop = [
    'Date and Time', 'Ch1 Size (µm)', 'Ch1 0.3µm (#)', 'Ch2 Size (µm)', 'Ch2 0.5µm (#)',
    'Ch3 Size (µm)', 'Ch3 1.0µm (#)', 'Ch4 Size (µm)', 'Ch4 3.0µm (#)', 'Ch5 Size (µm)',
    'Ch5 5.0µm (#)', 'Ch6 Size (µm)', 'Ch6 10.0µm (#)', 'Unnamed: 21', 'Unnamed: 22',
    'Unnamed: 23', 'total_count (#)'
]
...
SMPS_path = f'./burn_data/smps/MH_apollo_bed_05312024_NumConc.xlsx'  # Use user-specified date
...
def sum_columns(df, ranges):
    ...
    for start, end in ranges:
        col_range = [col for col in df.columns if str(col).replace('.', '', 1).isdigit() and start <= float(col) <= end]
...
new_df['timestamp_local'] = pd.to_datetime(new_df['timestamp_local'].str.replace('T', ' ').str.replace('Z', ''))

# Calculate min_from_peak for QuantAQ data
new_df['min_from_peak'] = (new_df['timestamp_local'] - start_time - pd.Timedelta(minutes=5.5)).dt.total_seconds() / 60
...
new_data_colors = ['#080be5', '#0068ff', '#0093ff', '#00b4ff', '#00d1d2', '#00eb7f', '#29ff06']
...
p.line([], [], legend_label="AeroTrak", color=None, line_width=0)  # Add a dummy line for AeroTrak
...
p.line([], [], legend_label="SMPS", color=None, line_width=0)  # Add a dummy line for SMPS
...
p.line([], [], legend_label="QuantAQ", color=None, line_width=0)  # Add a dummy line for QuantAQ
...
p.line([-8, -8], [1e-3, 1e5], line_color='black', line_width=2, legend_label="CR Boxes Activation")
