...
def combine_data_files(input_directory, output_filename):
    ...
    for filename in os.listdir(input_directory):
        if filename.endswith('.txt') and filename.startswith('MH_VOCUS_VavleAndRelayControl_'):
            file_path = os.path.join(input_directory, filename)
            # Read the tab-delimited file
            df = pd.read_csv(file_path, delimiter='\t')

            # Convert the datetime column to pandas datetime type
            df['datetime_EDT'] = pd.to_datetime(df['datetime_EDT'], format='%m/%d/%Y %I:%M:%S %p')
    ...
    mask = combined_df[columns_to_check].ne(combined_df[columns_to_check].shift()).any(axis=1)
    ...
    if 'GUV222_onOff' in combined_df.columns:
        ...
        guv222_mask = combined_df['GUV222_onOff'].ne(combined_df['GUV222_onOff'].shift())
        ...
        for index, row in changes_222.iterrows():
            print(f"Datetime: {row['datetime_EDT']}, GUV222_onOff: {row['GUV222_onOff']}")
    ...
    if 'GUV254_onOff' in combined_df.columns:
        ...
        guv254_mask = combined_df['GUV254_onOff'].ne(combined_df['GUV254_onOff'].shift())
        # Get the relevant rows where changes occur
        ...
        for index, row in changes_254.iterrows():
            print(f"Datetime: {row['datetime_EDT']}, GUV254_onOff: {row['GUV254_onOff']}")
