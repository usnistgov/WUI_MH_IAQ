...
if dataset == 'smps':
    df = pd.read_excel("./smps/MH_apollo_bed_04252024_MassConc.xlsx",sheet_name='all_data') #USER ENTERED FINAL PATH FOR RECS
...
data = np.random.rand(len(time_points), len(size_bins)) * 100  # Random data for illustration
...
p = figure(x_axis_label='Size Bins', y_axis_label='Time',
           x_range=size_bins, y_range=list(df.index.astype(str)))

# Create a color mapper
mapper = linear_cmap(field_name='Value', palette=Viridis256, low=df_stack['Value'].min(), high=df_stack['Value'].max())

# Plotting the rectangles (heatmap)
p.rect(x='Size', y='Time', width=1, height=1, source=ColumnDataSource(df_stack),
...
data = pd.read_excel('./burn_data/smps/MH_apollo_bed_05312024_MassConc.xlsx', 
...
da = xr.DataArray(data=data_filtered.values,  # use .values to get the underlying numpy array
...
cmap = mcolors.LinearSegmentedColormap.from_list('purple_green_red', ['purple', 'green', 'red'])
...
formatter.set_powerlimits((-3, 4))  # Use scientific notation for values outside this range
...
plt.gca().tick_params(axis='y', which='major', labelsize=12, width=2)  # Major ticks: larger text, thicker lines
plt.gca().tick_params(axis='y', which='minor', labelsize=8, width=1)   # Minor ticks: smaller text, thinner lines

plt.gca().tick_params(axis='x', which='major', labelsize=12, width=2)  # Major ticks: larger text, thicker lines
plt.gca().tick_params(axis='x', which='minor', labelsize=8, width=1)   # Minor ticks: smaller text, thinner lines
...
p = figure(width=800, height=500, title="Mass Concentration of Particles for Burn 10", 
           x_axis_label='Hours of the day', y_axis_label='Diameter (nm)',
           x_range=(0, max(time_hours)), y_range=(min(wavelength), max(wavelength)),
           x_axis_type='linear', y_axis_type='log')

# Add image
color_mapper = LinearColorMapper(palette=Viridis256, low=data_min, high=data_max)
p.image(image=[data_for_plot], x=0, y=min(wavelength), dw=max(time_hours), dh=max(wavelength), color_mapper=color_mapper)
