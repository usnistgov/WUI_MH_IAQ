...
from bokeh.models import ColumnDataSource, Band, Label, Arrow, OpenHead, CrosshairTool, Span, Legend, LegendItem, Div
...
if dataset == 'AeroTrakB':
    ...
    columns_to_drop = ['Date and Time', 'Ch1 Size (µm)', 'Ch2 Size (µm)', 'Ch2 0.5µm (#)', 
                        'Ch3 Size (µm)', 'Ch3 1.0µm (#)', 'Ch4 Size (µm)', 'Ch4 3.0µm (#)', 'Ch5 Size (µm)', 
                        'Ch5 5.0µm (#)', 'Ch6 Size (µm)', 'Ch6 10.0µm (#)', 'total_count (#)', 'Unnamed: 21', 'Unnamed: 22', 
                        'Unnamed: 23']
    ...
    df = reduce(lambda left, right: pd.merge(left, right, on=['min_since_peak'], how='outer'), data_frames)
    ...
    for i in range(2, 11):
        sheet_name = f'burn{i}'
        # Read and process the sheet
        df = pd.read_excel("./burn_dates_decay_aerotraks_kitchen.xlsx", sheet_name=sheet_name)
        df = df.drop(columns=columns_to_drop)
...
# Function to load and process CSV files
    ...
    bedroom_df = load_and_process_csv('./burn_data/quantaq/MOD-PM-00194-b0fc215029fa4852b926bc50b28fda5a.csv')
    kitchen_df = load_and_process_csv('./burn_data/quantaq/MOD-PM-00197-a6dd467a147a4d95a7b98a8a10ab4ea3.csv')
    ...
    def add_pm_total_column(df):
        ...
        if all(col in df.columns for col in required_columns):
            ...
            print(f"One or more of the required columns {required_columns} are missing in DataFrame.")
    ...
    def add_min_since_peak_column(df):
        if not df.empty and 'PM_total' in df.columns:
            ...
            df['min_since_peak'] = (df['timestamp_local'] - peak_time).dt.total_seconds() / 60
    ...
    burn_data = {burn: {'bedroom': pd.DataFrame(), 'kitchen': pd.DataFrame()} for burn in burn_dates.keys()}
...
if dataset == 'AeroTrakB':
    ...
    decay_data = {
        ...
        'Burn 2': df['burn2 total_count (#/cm³)'].values/df['burn2 total_count (#/cm³)'].max(),
        'Burn 3': df['burn3 total_count (#/cm³)'].values/df['burn3 total_count (#/cm³)'].max(),
        'Burn 4': df['burn4 total_count (#/cm³)'].values/df['burn4 total_count (#/cm³)'].max(),
        'Burn 5': df['burn5 total_count (#/cm³)'].values/df['burn5 total_count (#/cm³)'].max(),
        'Burn 6': df['burn6 total_count (#/cm³)'].values/df['burn6 total_count (#/cm³)'].max(),
        'Burn 7': df['burn7 total_count (#/cm³)'].values/df['burn7 total_count (#/cm³)'].max(),
        'Burn 8': df['burn8 total_count (#/cm³)'].values/df['burn8 total_count (#/cm³)'].max(),
        'Burn 9': df['burn9 total_count (#/cm³)'].values/df['burn9 total_count (#/cm³)'].max(),
        'Burn 10': df['burn10 total_count (#/cm³)'].values/df['burn10 total_count (#/cm³)'].max()
    }
    ...
    for label, data in decay_data.items():
        ...
        p.line(minutes_np, data, legend_label=label, line_width=line_width, color=color, line_dash=line_dash)
        ...
        p.line(minutes_np[fit_start_index:fit_end_index], y_curve_fit, line_color='red', line_dash='solid')  # No legend label
    ...
    for sheet_name in ['Burn 6', 'Burn 7', 'Burn 8', 'Burn 9', 'Burn 10']:
        ...
        # Plot the original data with legend
        p.line(minutes_np, normalized_data, legend_label=sheet_name, line_width=line_width, color=color, line_dash=line_dash)
    ...
    for burn in burn_dates.keys():
        ...
        dk = burn_data[burn]['bedroom']  # Assuming 'bedroom' is the data you want to analyze
        ...
        if not pm_total_at_15.empty:
            ...
            pm_total_at_15_value = np.nan  # Handle case where there is no data at exactly 15 minutes
        ...
        threshold_row = post_15_min_data[post_15_min_data['PM_total'] <= threshold_value]
        
        if threshold_row.empty:
            print(f"No PM_total values below 5% of maximum after 15 minutes for {burn}.")
            continue
        ...
        print(f"  Corresponding min_since_peak: {min_since_peak_at_threshold[burn]:.2f}")
        print()
    ...
    fit_ranges = {
        'Burn 5': (15, 250),
        'Burn 6': (0, 15),
        **{burn: (15, min_since_peak_at_threshold[burn]) for burn in burn_dates.keys() if burn not in ['Burn 5', 'Burn 6']}
    }
    ...
    for burn in burn_dates.keys():
        # Access the DataFrame from the dictionaries
        dk = burn_data[burn]['bedroom']  # Assuming 'bedroom' is the data you want to plot
        ...
        # Plot the original data with legend
        p.line(minutes_np, normalized_data, legend_label=burn, line_width=line_width, color=color, line_dash=line_dash)
        ...
        if len(x_fit) > 0 and len(y_fit) > 0:
            ...
            print(f"No data to fit for {burn} between {fit_start_min} and {fit_end_min} minutes.")
    ...
    for burn in burn_dates.keys():
        # Access the DataFrame for the current burn
        dk = burn_data[burn]['kitchen']  # Assuming 'kitchen' is the data you want to analyze
    ...
    for burn in burn_dates.keys():
        # Access the DataFrame from the dictionaries
        dk = burn_data[burn]['kitchen']  # Assuming 'kitchen' is the data you want to plot
    ...
    for label, data in decay_data.items():
        ...
        # Add Label with plain text
        p.add_layout(Label(x=label_x, y=label_y, text=fit_info, text_font='Calibri', text_font_size='12pt', text_align='left', text_baseline='middle'))
...
if dataset == 'AeroTrakB':
    ...
    crbox_mapping = {
        'Burn 1': 0, 'Burn 2': 4, 'Burn 3': 1, 'Burn 4': 1, 'Burn 5': 0,
        'Burn 6': 1, 'Burn 7': 2, 'Burn 8': 2, 'Burn 9': 2, 'Burn 10': 2
    }
    ...
    def calculate_cadr_and_uncertainty(row):
        ...
        if row['burn'] in ['Burn 6']:
            ...
            cadr_uncertainty = volume2 * ((row['decay'] + row['decay_uncertainty']) - (baseline_decay - baseline_decay_uncertainty)) - \
                                volume2 * ((row['decay'] - row['decay_uncertainty']) - (baseline_decay + baseline_decay_uncertainty))
        else:
            # Standard calculations for other burns
            cadr = volume1 * (row['decay'] - baseline_decay)
            cadr_uncertainty = volume1 * ((row['decay'] + row['decay_uncertainty']) - (baseline_decay - baseline_decay_uncertainty)) - \
                                volume1 * ((row['decay'] - row['decay_uncertainty']) - (baseline_decay + baseline_decay_uncertainty))
    ...
    baseline_decay_uncertainty = (burn_calc['decay_uncertainty'].iloc[3]**2 + burn_calc['decay_uncertainty'].iloc[4]**2)**0.5
    ...
    def calculate_cadr_and_uncertainty(row):
        if row['burn'] in ['Burn 5', 'Burn 6']:
            ...
            cadr_uncertainty = volume2 * ((row['decay'] + row['decay_uncertainty']) - (burn_calc['decay'].iloc[3] - burn_calc['decay_uncertainty'].iloc[3])) - \
                                volume2 * ((row['decay'] - row['decay_uncertainty']) - (burn_calc['decay'].iloc[3] + burn_calc['decay_uncertainty'].iloc[3]))
        else:
            # Standard calculations for other burns
            cadr = volume1 * (row['decay'] - burn_calc['decay'].iloc[0])
            cadr_uncertainty = volume1 * ((row['decay'] + row['decay_uncertainty']) - (burn_calc['decay'].iloc[0] - burn_calc['decay_uncertainty'].iloc[0])) - \
                                volume1 * ((row['decay'] - row['decay_uncertainty']) - (burn_calc['decay'].iloc[0] + burn_calc['decay_uncertainty'].iloc[0]))
        return pd.Series([cadr, cadr_uncertainty])
    ...
    burn_calc['CADR_uncertainty'] = volume * ((burn_calc['decay'] + burn_calc['decay_uncertainty']) - (burn_calc['decay'].iloc[0] - burn_calc['decay_uncertainty'].iloc[0])) - \
                                    volume * ((burn_calc['decay'] - burn_calc['decay_uncertainty']) - (burn_calc['decay'].iloc[0] + burn_calc['decay_uncertainty'].iloc[0]))

    # Calculate CADR per CRbox and uncertainty per CRbox
    burn_calc['CADR_per_CRbox'] = burn_calc['CADR'] / burn_calc['CRboxes'].replace(0, pd.NA)  # Avoid division by zero
    burn_calc['CADR_per_CRbox_uncertainty'] = burn_calc['CADR_uncertainty'] / burn_calc['CRboxes'].replace(0, pd.NA)  # Avoid division by zero
    ...
    def calculate_cadr_and_uncertainty(row):
        if row['burn'] in ['Burn 5']:
            ...
            cadr_uncertainty = volume2 * ((row['decay'] + row['decay_uncertainty']) - (burn_calc['decay'].iloc[1] - burn_calc['decay_uncertainty'].iloc[1])) - \
                                volume2 * ((row['decay'] - row['decay_uncertainty']) - (burn_calc['decay'].iloc[1] + burn_calc['decay_uncertainty'].iloc[1]))
        else:
            # Standard calculations for other burns
            cadr = volume1 * (row['decay'] - baseline_decay)
            cadr_uncertainty = volume1 * ((row['decay'] + row['decay_uncertainty']) - (baseline_decay - baseline_decay_uncertainty)) - \
    ...
    def calculate_cadr_and_uncertainty(row):
        if row['burn'] in ['Burn 5', 'Burn 6']:
            ...
            cadr_uncertainty = volume2 * ((row['decay'] + row['decay_uncertainty']) - (burn_calc['decay'].iloc[4] - burn_calc['decay_uncertainty'].iloc[4])) - \
                                volume2 * ((row['decay'] - row['decay_uncertainty']) - (burn_calc['decay'].iloc[4] + burn_calc['decay_uncertainty'].iloc[4]))
        else:
            # Standard calculations for other burns
            cadr = volume1 * (row['decay'] - burn_calc['decay'].iloc[0])
            cadr_uncertainty = volume1 * ((row['decay'] + row['decay_uncertainty']) - (burn_calc['decay'].iloc[0] - burn_calc['decay_uncertainty'].iloc[0])) - \
...
column2_plots = [shifted_plots[3], shifted_plots[4]]  # Plot 4 and Plot 5 (originally 5, 6)

# Create the layout
layout = column(row(*row1_plots),
                row(column2_plots[0], column2_plots[1], sizing_mode='scale_width'))
