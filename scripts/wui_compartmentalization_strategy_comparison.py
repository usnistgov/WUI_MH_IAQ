...
def process_aerotrak_data(file_path, instrument='AeroTrakB'):
    ...
    filtered_data = aerotrak_data[aerotrak_data['Date'].isin(figure4_burn_dates.dt.date)].copy()
    ...
    for burn_id in figure4_burn_ids:
        ...
        if not matching_rows.empty:
            datetime_values = pd.to_datetime(filtered_data.loc[
            ...
            time_since_closed = (datetime_values - garage_closed_time).dt.total_seconds() / 3600
            filtered_data.loc[matching_rows.index, 'Time Since Garage Closed (hours)'] = time_since_closed
...
def process_dusttrak_data(file_path):
    ...
    filtered_data = dusttrak_data[dusttrak_data['Date'].isin(figure4_burn_dates.dt.date)].copy()
    
    # Apply time shifts and calculate time since garage closed
    for burn_id in figure4_burn_ids:
        if burn_id in burn_log['Burn ID'].values:
            burn_date = burn_log[burn_log['Burn ID'] == burn_id]['Date'].values[0]
            filtered_data = apply_time_shift(filtered_data, 'DustTrak', burn_id, burn_date)
...
def process_purpleairk_data(file_path):
    ...
    figure4_burn_ids = [burn_id for burn_id in BURN_GROUPS['figure4'] if int(burn_id.replace('burn', '')) >= 6]
    figure4_burn_dates = burn_log[burn_log['Burn ID'].isin(figure4_burn_ids)]['Date']
    figure4_burn_dates = pd.to_datetime(figure4_burn_dates)
    
    purpleair_data['Date'] = pd.to_datetime(purpleair_data['DateTime']).dt.date
    filtered_data = purpleair_data[purpleair_data['Date'].isin(figure4_burn_dates.dt.date)].copy()
...
def process_quantaq_data(file_path, instrument):
    ...
    filtered_data = quantaq_data[quantaq_data['Date'].isin(figure4_burn_dates.dt.date)].copy()
        ...
        try:
            ...
            if not burn_id_row.empty:
                ...
                result_df = pd.DataFrame({
                    ...
                    'burn_id': burn_id,
                    'Total Concentration (µg/m³)': pd.to_numeric(smps_data['Total Concentration (µg/m³)'], errors='coerce')
                })
    ...
    for burn_id in burn_ids:
        ...
        for pollutant in pollutants:
            ...
            if pd.isna(max_concentration):
                print(f"No valid data for {pollutant} on {burn_id}, skipping decay calculation.")
                continue
            
            max_index = burn_data[pollutant].idxmax()
            max_time = burn_data['Time Since Garage Closed (hours)'].iloc[max_index - burn_data.index[0]]
            ...
            if burn_special_case.get('custom_decay_time', False):
                ...
                decay_start_time = max(0, max_time)  # Start at garage closing or peak, whichever is later
                
                # Adjust for CR Box activation if applicable
                cr_box_on_time_str = burn_info['CR Box on'].iloc[0]
                if pd.notna(cr_box_on_time_str):
                    cr_box_on_time = create_naive_datetime(burn_info['Date'].iloc[0], cr_box_on_time_str)
                    garage_closed_time = create_naive_datetime(burn_info['Date'].iloc[0], garage_closed_time_str)
                ...
                for i in range(len(sorted_data)):
                    current_time = sorted_data['Time Since Garage Closed (hours)'].iloc[i]
                    if current_time >= decay_start_time and i >= 5:
                        rolling_mean = sorted_data['rolling_derivative'].iloc[i-3:i+1].mean()
                        stability_threshold = 0.1
                        if abs(sorted_data['rolling_derivative'].iloc[i] - rolling_mean) < stability_threshold:
            ...
            try:
                ...
                print(f"  Decay Rate: {popt[1]:.4f} ± {1.96 * perr[1]:.4f} h⁻¹ (RSD: {rsd:.2f})")
...
def plot_figure4_raw_data(data, instrument, output_to_file=False):
    ...
    for burn_id in burns_to_plot:
        ...
        if instrument == 'SMPS':
            ...
            for pollutant in pollutants:
                if pollutant in burn_data.columns and not burn_data[pollutant].isna().all():
        ...
        if burn_id in decay_parameters and selected_pollutant in decay_parameters[burn_id]:
    ...
    p.line(x=[0, 0], y=[10**-2, 10**4], line_color='black', line_width=1.5, 
           line_dash='solid', legend_label='Garage Closed')
...
if __name__ == "__main__":
    ...
    output_to_file = True  # Set to True to save as HTML file, False for notebook display only
